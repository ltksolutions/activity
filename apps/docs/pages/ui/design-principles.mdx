...
Kompletný brand manuál je v [brand](brand) — farby, typografia, logo, ikonografia, použitie. Tu len krátky prehľad pre kontext dizajnových princípov.
...
Pre slovenskú diakritiku treba zahrnúť **`latin + latin-ext`** subsety. Detail v [brand](brand#slovenská-diakritika).
...
Brand v2 používa **slovenskú národnú farebnosť** — SK Blue ako primárna, SK Red ako akcent. Plný popis vrátane HSL hodnôt, dark mode variantov a pravidiel použitia v [brand](brand#farebná-paleta).
...
Neutral 950: --ink        headlines
```

Dark mode má samostatné odtiene (`--sk-blue-light`, `--sk-red-light`, Ink ↔ Paper swap), ale rovnakú sémantiku.
...
V `branding-source/activity-icons.html` je 32 **custom UI ikon** ako kandidáti pre prípadný vlastný icon set v `packages/ui/icons/`. Pre MVP postačujú Lucide ikony — pozri [brand](brand#ikonografia).
...
Pre kompletný brand manuál pokračuj v [brand](brand).
Pre konkrétne mockupy pokračuj v [mockups](mockups).
Pre architektúru pokračuj v [../architecture](/architecture).
